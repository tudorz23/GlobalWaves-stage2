package commands.userCommands.hostCommands;

import client.Session;
import com.fasterxml.jackson.databind.node.ArrayNode;
import commands.ICommand;
import database.audio.Podcast;
import database.users.Host;
import database.users.User;
import fileio.input.CommandInput;
import fileio.output.PrinterBasic;
import utils.enums.UserType;

public class RemovePodcastCommand implements ICommand {
    private final Session session;
    private final CommandInput commandInput;
    private final User user;
    private final ArrayNode output;

    /* Constructor */
    public RemovePodcastCommand(final Session session, final CommandInput commandInput,
                                final User user, final ArrayNode output) {
        this.session = session;
        this.commandInput = commandInput;
        this.user = user;
        this.output = output;
    }

    @Override
    public void execute() {
        session.setTimestamp(commandInput.getTimestamp());
        PrinterBasic printer = new PrinterBasic(output, commandInput);

        if (user.getType() != UserType.HOST) {
            printer.print(user.getUsername() + " is not a host.");
            return;
        }

        Podcast podcast;
        Host host = (Host) user;

        try {
            podcast = host.findPodcast(commandInput.getName());
        } catch (IllegalArgumentException exception) {
            printer.print(exception.getMessage());
            return;
        }

        session.getDatabase().simulateTimeForEveryone(session.getTimestamp());

        if (!session.getDatabase().canRemoveAudio(podcast)) {
            printer.print(host.getUsername() + "  can't delete this podcast.");
            return;
        }

        host.removePodcast(podcast);
        session.getDatabase().removePodcast(podcast);

        printer.print(host.getUsername() + " deleted the podcast successfully.");
    }
}
